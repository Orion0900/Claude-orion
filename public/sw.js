 ...
 */
const VERSION = 'v1'
const SHELL_CACHE = `loopmaker-shell-${VERSION}`
const TILE_CACHE = `loopmaker-tiles-${VERSION}`
...
const LIVE_ONLY_HOSTS = ['routing.openstreetmap.de', 'api.open-meteo.com', 'nominatim.openstreetmap.org']
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    (async () => {
      ...
      await Promise.all(
        names
          .filter((name) => name.startsWith('loopmaker-') && !name.endsWith(VERSION))
