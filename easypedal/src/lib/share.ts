...
/**
 * Apple Maps walking directions to the start of a ride.
 *
 * Apple's URL scheme carries a single destination and no way to describe the
 * roads in between, so it cannot be handed this route — that is what the GPX
 * and the in-app navigation are for. What it can do is get you to the start.
 */
export function appleMapsUrl(start: { lat: number; lng: number }, label = 'Ride start'): string {
