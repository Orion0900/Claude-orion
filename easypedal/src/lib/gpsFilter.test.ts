...
describe('createFixFilter', () => {
  ...
  it('accepts a rider moving at a rider’s pace', () => {
    const filter = createFixFilter()
    filter.accept(fix({ timestamp: 0 }))
    // 8 m/s is a brisk but entirely ordinary cycling speed.
    const later = fix({ position: destination(home, 90, 8), timestamp: 1000 })
  ...
  it('refuses a jump no rider could make', () => {
    const filter = createFixFilter()
    filter.accept(fix({ timestamp: 0 }))
