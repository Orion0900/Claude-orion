/**
 * Keeping rides you liked.
 *
 * A commute is ridden again and again, and the map data behind it can shift
 * between searches. Saved rides are stored whole — geometry, elevation, road
 * types and turn instructions — which also means a saved ride can be started
 * with no connection at all, since nothing has to be routed again.
 */
...
/**
 * Identity for "the same ride", independent of the search that produced it.
 * Route ids come from the position in a particular search, so they differ
 * between searches even when the ride is identical.
 */
...
/**
 * A default name a rider would recognise: "Home to Work, 8 Sep".
 *
