...
export function SavedRoutes({ routes, distanceUnit, elevationUnit, onOpen, onRemove }: SavedRoutesProps) {
  ...
  return (
    <section className="panel-section">
          ...
          <li key={saved.id}>
            <button type="button" className="saved-open" onClick={() => onOpen(saved.route)}>
              ...
              <span className="saved-stats">
                ...
                {saved.route.ways ? ` · ${Math.round(saved.route.bikewayShare * 100)}% lanes` : ''}
              </span>
            </button>
            <button
              type="button"
      ...
      <p className="hint">Saved rides work without a connection — no searching needed.</p>
    </section>
  )
}
