...
import { buildRideSummary, summaryHeadline } from '../lib/rideSummary'
...
interface NavigationViewProps {
  ...
  /** Easy cruising speed, in the distance unit per hour. */
  speed: number
  onHeading: (heading: number | null) => void
  onPosition: (position: LatLng | null) => void
  onProgress: (fraction: number) => void
  /** True while the rider has dragged the map away to look around. */
  browsing: boolean
  onRecenter: () => void
...
export function NavigationView({
  ...
  // Wall-clock start, so the summary reports time actually spent riding.
  ...
  // Once per ride: the ride is one mount of this component.
  useEffect(() => {
  ...
  // included. Permission was asked for on the tap that started the ride.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const watch = navigator.geolocation.watchPosition(
      (fix) => {
        ...
        // so the worst fixes arrive exactly when a rider turns.
        ...
        // Wait for a second far-off match before believing the rider really is
        ...
        setError(null)
        setProgress(next)
        // On the route, the rider is drawn on it: raw fixes jitter by metres
        ...
        // rider is facing comes from the compass, below.
        courseRef.current =
  ...
  // The end of a ride is the one moment a rider is certain to look at the
    ...
    return (
      <div className="nav nav-finished">
        <div className="finish-card">
          ...
          <dl className="finish-stats four">
            <div>
              <dt>Time</dt>
            ...
            <div>
              <dt>Avg speed</dt>
              <dd>
                {summary.averageSpeed === null ? '—' : `${summary.averageSpeed.toFixed(1)} ${distanceUnit}/h`}
              </dd>
            </div>
            <div>
              <dt>Climb</dt>
          ...
          <div className="finish-actions">
            <button type="button" className="btn" onClick={onToggleSaved}>
              {isRouteSaved ? 'Saved ✓' : 'Save this ride'}
            </button>
            <button
              type="button"
    ...
    <div className="nav">
        ...
        {upcoming && metersToTurn !== null ? (
          ...
          <div className="nav-banner-text">
            <span className="nav-instruction">
              {finished ? 'You’ve arrived — nice ride' : progress ? 'Carry on' : 'Finding you…'}
            </span>
          </div>
        )}
      </div>
      ...
      {strayed && progress ? (
        <p className="nav-alert">
          {Math.round(progress.offRouteBy)} m off route — head back to the line
        </p>
      ) : null}
